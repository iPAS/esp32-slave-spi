...
void SlaveSPI::begin(gpio_num_t so, gpio_num_t si, gpio_num_t sclk, gpio_num_t ss, size_t length, void (*ext)()) {
    ...
    driver = new spi_slave_transaction_t {
        ...
        .tx_buffer = heap_caps_malloc(max(t_size, 32), SPI_MALLOC_CAP), //< tx buffer, or NULL for no MOSI phase
        .rx_buffer = heap_caps_malloc(max(t_size, 32), SPI_MALLOC_CAP), //< rx buffer, or NULL for no MISO phase
        .user      = NULL        //< User-defined variable. Can be used to store e.g. transaction ID.
        };
    for (int i = 0; i < t_size; i++)
        ((char *)driver->rx_buffer)[i] = 0;  
    ...
    esp_err_t err;
    err = spi_slave_initialize(spi_host, &buscfg, &slvcfg, SPI_DMA);
    err = spi_slave_queue_trans(spi_host, driver, portMAX_DELAY);  // ready for input (no transmit)
